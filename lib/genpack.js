...
for (const emojiSymbol of emojiSymbols) {
  emojiKeywords[emojiSymbol].forEach(keyword => {
    if (Object.hasOwn(keywordsMap, keyword) === false) {
      keywordsMap[keyword] = []
    }
    keywordsMap[keyword].push(emojiSymbol)
  })

  if (Object.hasOwn(emojiData, emojiSymbol) === false) {
    const codepoint = getCodePoint(emojiSymbol)
    console.log(`skipping '${codepoint}' (${emojiSymbol}): not present in unicode-emoji-json`)
    continue
  }
